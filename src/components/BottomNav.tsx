...
import { Receipt, BarChart3, Plus, FolderTree } from 'lucide-react';
import { playActionClick } from '../utils/audio';
import { triggerHaptic } from '../utils/haptics';

export type MobileTab = 'ledger' | 'dashboard' | 'numpad' | 'categories';
...
export const BottomNav: React.FC<BottomNavProps> = ({ currentTab, onChangeTab }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-[#FAF9F6]/95 backdrop-blur-md border-t border-[#E6E2DA] pb-safe sm:hidden isolate">
      <div className="flex items-center justify-around px-2 h-[52px] max-w-md mx-auto">
        {/* Tab 1: Sổ cái */}
        <button
          onClick={() => handleTabClick('ledger')}
          className={`flex flex-col items-center justify-center min-w-[54px] h-full transition-colors tactile-btn ${
